...
<script>
// //페이징처리
// function pagingWorkAjax(page, pageSize) {
// 	$.ajax({
...
//     google.charts.load('current', {'packages':['gantt']});
    ...
    function drawChart() {
      ...
      data.addRows([
    	  ...
    	  ['${status.count}', '${pmsChartList.work_nm}', 'winter',
    	         new Date(<fmt:formatDate value="${pmsChartList.s_dt}" pattern="yyyy,MM,dd"/>),
    	         new Date(<fmt:formatDate value="${pmsChartList.e_dt}" pattern="yyyy,MM,dd"/>+1), null, ${pmsChartList.prog}, null]
	...
	<div class="panel panel-default">
		...
		<div class="panel-body" style="display: block; max-height: 600px; min-height: 600px; overflow :auto;">
				<c:forEach items="${empList}" var="emp" varStatus="loop">
					...
					<td>
						<!-- 구성원 프로필 이미지 -->
						<img id="pictureViewImg" style="width: 55px; height: 55px; margin-right: 20px; float: left;" src="${cp}/empController/profile?emp_id=${detailUser.emp_id}" />
					</td>
					<td>
						<!--  구성원 이름 / 직급 / 부서명 -->
						<h4 style="margin-top: 17px;">${emp.ko_nm} &nbsp;/&nbsp; ${emp.ko_nm} / ${emp.ko_nm}</h4>
					</td>
